...
def main() -> None:
    repo = _repo_root()
    raw_dir = repo / "data" / "cmapss_fd001" / "raw"
    out_dir = repo / "data" / "cmapss_fd001" / "prepared"
    ...
    train_df.to_csv(out_dir / "train_fd001.csv", index=False)
    test_df.to_csv(out_dir / "test_fd001.csv", index=False)
    rul_df.to_csv(out_dir / "rul_fd001.csv", index=False)
    ...
    print(f"Prepared FD001 files written to:\n{out_dir}")
