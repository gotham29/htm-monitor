...
# A practical first-pass subset for demoing multivariate degradation.
DEFAULT_DEMO_SENSORS = [
    "sensor_7",
    "sensor_8",
    "sensor_9",
    "sensor_11",
...
def _load_prepared_fd001(prepared_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    test_path = prepared_dir / "test_fd001.csv"
    rul_path = prepared_dir / "rul_fd001.csv"
    ...
    _required_columns_present(test_df, ["unit_id", "cycle"], ctx="test_fd001.csv")
    _required_columns_present(rul_df, ["test_unit_id", "rul"], ctx="rul_fd001.csv")
...
def _sensor_variability_summary(test_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not sensor_cols:
        raise ValueError("No sensor_* columns found in test_fd001.csv")
...
def main() -> None:
    ...
    demo_dir = repo / "data" / "cmapss_fd001" / "demo"
    demo_dir.mkdir(parents=True, exist_ok=True)

    test_df, rul_df, prep_meta = _load_prepared_fd001(prepared_dir)
    ...
    scored_path = demo_dir / "fd001_test_scored.csv"
    engine_summary_path = demo_dir / "fd001_engine_summary.csv"
    sensor_summary_path = demo_dir / "fd001_sensor_summary.csv"
    metadata_path = demo_dir / "fd001_demo_metadata.json"

    scored_df.to_csv(scored_path, index=False)
    engine_df.to_csv(engine_summary_path, index=False)
    sensor_summary_df.to_csv(sensor_summary_path, index=False)

    available_demo_sensors = [s for s in DEFAULT_DEMO_SENSORS if s in scored_df.columns]

    metadata = {
        "source_dataset": "NASA CMAPSS FD001",
        "prepared_inputs": {
            "test_csv": str(prepared_dir / "test_fd001.csv"),
            "rul_csv": str(prepared_dir / "rul_fd001.csv"),
            "metadata_json": str(prepared_dir / "metadata.json"),
        },
        "artifacts": {
            "fd001_test_scored_csv": str(scored_path),
            "fd001_engine_summary_csv": str(engine_summary_path),
            "fd001_sensor_summary_csv": str(sensor_summary_path),
        },
        "row_counts": {
        ...
        "recommended_demo_sensors": available_demo_sensors,
    ...
    print("Built CMAPSS FD001 demo artifacts:")
    print(f"  {scored_path}")
